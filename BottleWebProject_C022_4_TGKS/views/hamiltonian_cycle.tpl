...
<div class="row">
    <div class="col-6 d-flex align-items-stretch" id="cardOne">
        <div class="card p-4 shadow w-100">
            <div class="card-body">
                <h3 class="card-header text-center pt-0 mb-2"><strong>Исходные данные</strong></h3>

                <form method="post" id="form" action="/hamiltonian_cycle" novalidate autocomplete="off">
                    <label for="amountOfVertices" class="fs-5 form-label">Введите количество вершин:</label>
              
                    <input type="text" name="amountOfVertices" class="form-control" id="amountOfVertices" value="">
                    ...
                    <input type="submit" class="btn btn-dark" id="getRand" name="getRand" value="Сгенерировать граф">
                    <input type="submit" class="btn btn-dark" id="getAns" name="getAns" value="Решить">
                </form>
            </div>
        </div>
    </div>

    <div class="col-6 d-flex align-items-stretch" id="cardTwo">
        <div class="card p-4 shadow w-100">
            <div class="card-body">
                ...
                %if answer != []:
                    <div class="text-center" id="answers">
                        ...
                        %if answer == ["В данном графе нет Гамильтоновых циклов!"]:
                            <p class="fs-5 ans">В данном графе нет Гамильтоновых циклов!</p>
                        %else:
                            %counter = 1
                            %for i in answer:
                                <p class="fs-5 ans">Цикл {{counter}}: {{' '.join([str(elem) for elem in i])}}</p>
                                %counter += 1
                            %end
                        %end
                        %end
...
<script>
    $(document).ready(function() {
        ...
        $("#getAns").attr("disabled", "disabled")
    })

    if ($("#graph").attr("src"))
    {
        $("#rowImg").removeClass("d-none")
    }
    else
    {
        $("#rowImg").addClass("d-none")
    }
